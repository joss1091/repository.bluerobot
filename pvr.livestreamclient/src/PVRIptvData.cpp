...
bool PVRIptvData::LoadPlayList(void)
{
  ...
  if (!GetCachedFileContents(M3U_FILE_NAME, m_strM3uUrl, strPlaylistContent, g_bCacheM3U))
  {
    ...
    // XBMC->QueueNotification(QUEUE_ERROR, "Ocurrio un error al obtener la lista de canales.Intenta mas tarde");
    if (GUI->Dialog_YesNo_ShowAndGetInput("Error al cargar canales",
         "", "Ocurrio un error al intentar cargar los canales", "Se va a intentar cargar nuevamente los canales, presione el boton para intentar nuevamente.",
         "", "Reintentar"))
         {
           LoadPlayList();
         }
