 ...
 *      http://github.com/afedchin/xbmc-addon-livestreamclient/
 *
...
#define DEVICE_NOT_FOUND "device_not_found"
...
bool PVRIptvData::LoadPlayList(void)
{
  ...
  if (!GetCachedFileContents(M3U_FILE_NAME, m_strM3uUrl, strPlaylistContent, g_bCacheM3U))
  {
    ...
    XBMC->QueueNotification(QUEUE_ERROR, "Ocurrio un error al obtener la lista de canales");
    kodi::gui::dialogs::OK::ShowAndGetInput("TV", "Ocurrio un error al obtener la lista de canales");
    bool canceled;
    bool ret = kodi::gui::dialogs::YesNo::ShowAndGetInput(
       "TV",   /* The Header */
       "Ocurrio un error al obtener lista de canales, deseas intentar nuevamente?",
       canceled);            /* Yes label, also optional and if empty "Yes" */
  ...
  while(std::getline(stream, strLine))
  {
    ...
    if (bFirst)
    {
      ...
      if (StringUtils::Left(strLine, (int)strlen(NO_SUBSCRIPTION)) == NO_SUBSCRIPTION)
      {
        ...
        kodi::gui::dialogs::OK::ShowAndGetInput("Suscripcion", "No cuentas con una subscripcion");
        continue;
      }
      if (StringUtils::Left(strLine, (int)strlen(DEVICE_NOT_FOUND)) == DEVICE_NOT_FOUND)
      {
        XBMC->QueueNotification(QUEUE_ERROR, "No se encontro el dispositivo.");
        kodi::gui::dialogs::OK::ShowAndGetInput("Error", "No se encontro el dispositivo\n Contacta a tu proveedor.");
        continue;
      }
      else
      {
